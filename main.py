...
from dlmisc.metrics import (minimum_sensitivity, write_array_to_file,
                            write_metrics_dict_to_file)
from dlmisc.results import write_results_file, load_results
...
from functions import (compute_binary_metrics, compute_regression_metrics,
                       fix_seeds, load_sigma_dataset, print_binary_metrics,
                       print_regression_metrics)
from hydra_minirocket_classifier import (HydraMultivariateClassifier,
...
def cfg():
    ...
    if fold >= n_folds:
        raise ValueError(f'Fold {fold} must be smaller than the number of folds ({n_folds})')
...
@ex.automain
def run(seed, n_folds, fold, dataset_path, method, task, temporal, output_paths, ridge_alphas, _config):
    # Do not repeat a experiment with the exact same config if already run
    results = load_results(output_paths['results'])
    if results is not None:
        if results.find_experiment(_config) is not None:
    ...
    if task == 'regression':
        ...
        train_x, test_x, train_y, test_y = X[train_idx], X[test_idx], y[train_idx, 0], y[test_idx, 0]
        ...
        train_x, test_x, train_y, test_y = X[train_idx], X[test_idx], y[train_idx, -1], y[test_idx, -1]
    ...
    if not temporal:
        ...
        train_x_new = train_x.transpose((0, 2, 1)).reshape(train_x.shape[0] * train_x.shape[2], train_x.shape[1])
        
        # Check that the values new array and the old matches
        for i in range(train_x.shape[0]):
            for j in range(train_x.shape[2]):
                assert np.allclose(train_x_new[i * train_x.shape[2] + j], train_x[i, :, j])

        train_y = np.repeat(train_y, train_x.shape[2])
        train_groups = np.repeat(train_groups, train_x.shape[2])
        train_x = train_x_new

        test_x_new = test_x.transpose((0, 2, 1)).reshape(test_x.shape[0] * test_x.shape[2], test_x.shape[1])
        test_y = np.repeat(test_y, test_x.shape[2])
        test_groups = np.repeat(test_groups, test_x.shape[2])
        test_x = test_x_new

    print(f'Data loaded and splitted. Train size: {train_x.shape}, Test size: {test_x.shape}')
    ...
    if task == 'regression':
        ...
        scorer = make_scorer(
            lambda y_true, y_pred: minimum_sensitivity(y_true, y_pred) * accuracy_score(y_true, y_pred),
            # roc_auc_score,
            greater_is_better=True
        )
        final_clf = RidgeClassifierCV(alphas=ridge_alphas, scoring=scorer, class_weight='balanced')
    ...
    if method == 'hydra':
        ...
        ts_clf = GridSearchCV(
            ...
            param_grid = {
                'num_features' : [250, 500, 1000, 2000, 4000]
            },
            scoring=scorer,
            n_jobs=1,
            cv=cv,
        ...
        ts_clf = GridSearchCV(
            estimator=RocketMultivariateClassifier(
                clf = final_clf,
                verbose=0
            ),
            param_grid = {
